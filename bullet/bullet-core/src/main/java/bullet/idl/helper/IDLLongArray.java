/*
  -------------------------------------------------------
   This file was generated by JParser
 
   Do not make changes to this file
  -------------------------------------------------------
*/
package bullet.idl.helper;

public class IDLLongArray extends IDLArray {

    public static final IDLLongArray NULL = native_new();

    /**
     * @return An empty instance without a native address
     */
    public static IDLLongArray native_new() {
        return new IDLLongArray((byte) 1, (char) 1);
    }

    protected IDLLongArray(byte b, char c) {
        super((byte) 1, (char) 1);
    }

    protected IDLLongArray() {
        super((byte) 1, (char) 1);
    }

    public IDLLongArray(int size) {
        super((byte) 1, (char) 1);
        long addr = internal_native_create_int(size);
        internal_reset(addr, true);
    }

    public void copy(long[] array) {
        int length = array.length;
        resize(length);
        for (int i = 0; i < length; i++) {
            long value = array[i];
            setValue(i, value);
        }
    }

    /*
      [-JNI;-NATIVE]
      return (jlong)new IDLLongArray((int)size);
    */
    public static native long internal_native_create_int(int size);

    protected void deleteNative() {
        internal_native_deleteNative(native_address);
    }

    /*
      [-JNI;-NATIVE]
      IDLLongArray* nativeObject = (IDLLongArray*)this_addr;
      delete nativeObject;
    */
    public static native void internal_native_deleteNative(long this_addr);

    public long getValue(int index) {
        return internal_native_getValue(native_address, index);
    }

    /*
      [-JNI;-NATIVE]
      IDLLongArray* nativeObject = (IDLLongArray*)this_addr;
      return nativeObject->getValue((int)index);
    */
    public static native long internal_native_getValue(long this_addr, int index);

    public void setValue(int index, long value) {
        internal_native_setValue(native_address, index, value);
    }

    /*
      [-JNI;-NATIVE]
      IDLLongArray* nativeObject = (IDLLongArray*)this_addr;
      nativeObject->setValue((int)index, value);
    */
    public static native void internal_native_setValue(long this_addr, int index, long value);

    public void copy(IDLLongArray src, int srcOffset, int destOffset, int length) {
        internal_native_copy(native_address, src.native_address, srcOffset, destOffset, length);
    }

    /*
      [-JNI;-NATIVE]
      IDLLongArray* nativeObject = (IDLLongArray*)this_addr;
      nativeObject->copy(*((IDLLongArray* )src_addr), (int)srcOffset, (int)destOffset, (int)length);
    */
    public static native void internal_native_copy(long this_addr, long src_addr, int srcOffset, int destOffset, int length);
}
